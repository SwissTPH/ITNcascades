...
data_bit103_collapsed<-read_excel(file.path(dataDir,"BIT103_All_Data_AAA_II.xlsx"), sheet="EH_All_Data_Collapsed")

netcodes_103=data_bit103_collapsed%>% select(netcode, arm, Nettype, wash, ITN)%>%
  rename(test_item_number=netcode)%>% unique()

huttypes=data_bit103_collapsed%>% select(HutType, hut_number)%>%
  unique()

#Magnet: 81 and 82 (unwashed and washed)
#OlysetPlus: 81 and 82 (unwashed and washed)
#Magnet: 61 and 62 (unwashed and washed)
#IG2: 71 and 72 (unwashed and washed)
# control : 50
...
data_bit103=data_bit103_0%>%
  ...
  ungroup()%>%
  merge(netcodes_103)%>% 
  filter(ITN %in% c("Control", "MagNet_ITN", "Olyset_Plus", "Interceptor_G2"))%>% 
  mutate(treatment=ifelse(ITN=="MagNet_ITN", 1,
                          ifelse(ITN=="Olyset_Plus", 2,
                                 ifelse( ITN=="Interceptor_G2", 3, 
                                         ifelse( ITN=="Control", 0, NA)))), 
         wash=ifelse(ITN %in% c("Control"), "Control", wash),
...
```{r }
data_bit103_all_72=data_bit103 %>%
  ...
  select(day, wash_status, insecticide_name, treatment,UA, UD, FA, FD, dead, fed, total)


data_bit103_all_24=data_bit103 %>%
  filter(mosquitoes_strain_name=="Anopheles gambiae s.l") %>%
  rename(UD_12=UD, UA_12=UA, FA_12=FA, FD_12=FD)%>%
  mutate(UA=UA_24,
         FA=FA_24,
...
write.csv(data_bit103_all_72, file = file.path(outputsDir, "assenga_all_72.csv"), row.names = F)
write.csv(data_bit103_all_24, file = file.path(outputsDir, "assenga_all_24.csv"), row.names = F)


```

## Assenga et al., Cote d'Ivoire
Import dataset, rename and select main variables

```{r  warning=FALSE ,  results='hide'}
data_103_CI_unwashed<-read_excel(file.path(dataDir,"Rawdata_Hut Phase 2 An gambiae.xlsx"), sheet="72h_NL_Agamb")
data_103_CI_washed<-read_excel(file.path(dataDir,"Rawdata_Hut Phase 2 An gambiae.xlsx"), sheet="72h_L20_Agamb")


data_103_CI_unwashed_24<-read_excel(file.path(dataDir,"Rawdata_Hut Phase 2 An gambiae.xlsx"), sheet="24h_NL_Agamb")
data_103_CI_washed_24<-read_excel(file.path(dataDir,"Rawdata_Hut Phase 2 An gambiae.xlsx"), sheet="24h_L20_Agamb")

unique(data_103_CI_unwashed$treat)
table(data_103_CI_washed$treat)

netcodes_NL=data.frame(treat=c("EMP-CTRL-NL","0104112022-NL", "0119052023-NL", "0112062023-NL"),
           insecticide_name=c("Control","Magnet", "OlysetPlus", "IG2"), 
           treatment=c("0","1", "2", "3"), 
           wash_status="unwashed")
netcodes_L20=data.frame(treat=c("EMP-CTRL-L20x","1504112022-L20x", "0119052023-L20x","0112062023-L20x"), 
           treatment=c("0","1", "2", "3"), 
           insecticide_name=c("Control","Magnet", "OlysetPlus", "IG2"),
           wash_status="20x")


combined=rbind(data_103_CI_unwashed %>% right_join(netcodes_NL)%>% 
             select("Round" ,"day" , "hut" , "treat" , "sleeper","insecticide_name","treatment",
                    "unf_live","unf_dead","bf_live", "bf_dead","wash_status"
                    #"tot_unf","tot_blf","tot_live", "tot_dead"
                    ),
           data_103_CI_washed %>% right_join(netcodes_L20)%>% 
             select("Round" ,"day" , "hut" , "treat" , "sleeper","insecticide_name","treatment",
                    "unf_live","unf_dead","bf_live", "bf_dead","wash_status"
                    #"tot_unf","tot_blf","tot_live", "tot_dead"
                    ))%>%
  mutate(fed=bf_live+bf_dead, dead=unf_dead+bf_dead,
         total=bf_live+bf_dead+unf_dead+unf_live#,
         #test_dead=dead-tot_dead, test_fed=fed-tot_blf, test_tot=total-tot_live-tot_dead
         )%>%
  rename(UA="unf_live", UD="unf_dead", FD="bf_dead", FA="bf_live")

# data consistency checks
data_103_CI_compare=read_dta(  file=file.path(dataDir,"BIT103 IC all data.dta"))
test=combined%>%left_join(data_103_CI_compare %>%filter(time==72))

summary(test$fed-test$tot_blf)
summary(test$dead-test$tot_dead)
summary(test$total-test$fed-test$tot_unf)
summary(test$total-test$dead-test$tot_live)


summary(test$total-test$UA-test$UD-test$FD-test$FA)
summary(test$total-test$UA-test$UD-test$FD-test$FA)


# 24h

combined_24=rbind(data_103_CI_unwashed_24 %>% right_join(netcodes_NL)%>% 
             select("Round" ,"day" , "hut" , "treat" , "sleeper","insecticide_name","treatment",
                    "unf_live","unf_dead","bf_live", "bf_dead","wash_status"
                    #"tot_unf","tot_blf","tot_live", "tot_dead"
                    ),
           data_103_CI_washed_24 %>% right_join(netcodes_L20)%>% 
             select("Round" ,"day" , "hut" , "treat" , "sleeper","insecticide_name","treatment",
                    "unf_live","unf_dead","bf_live", "bf_dead","wash_status"
                    #"tot_unf","tot_blf","tot_live", "tot_dead"
                    ))%>%
...
# data consistency checks
test24=combined_24%>%left_join(data_103_CI_compare %>%filter(time==24))

summary(test24$fed-test24$tot_blf)
summary(test24$dead-test24$tot_dead)
summary(test24$total-test24$fed-test24$tot_unf)
summary(test24$total-test24$dead-test24$tot_live)


data_bit103_CI_72=combined%>%
  select(day, insecticide_name, treatment, UA, UD, FA, FD, wash_status, fed, dead, total)

data_bit103_CI_24=combined_24%>%
  select(day, insecticide_name, treatment, UA, UD, FA, FD, wash_status, fed, dead, total)
...
```{r ,  results='hide' , warning=FALSE}
write.csv(data_bit103_CI_72, file = file.path(outputsDir, "assenga_cotedivoire_72.csv"), row.names = F)
write.csv(data_bit103_CI_24, file = file.path(outputsDir, "assenga_cotedivoire_24.csv"), row.names = F)
