...
estimates_bit103_24=read.csv(file.path(outputsDir, "estimates_assenga_24_all.csv"))

estimates_bit103_24_CI=read.csv(file.path(outputsDir, "estimates_assengaCotedIvoire_24.csv"))


estimates_martin_24=read.csv(file.path(outputsDir, "estimates_martin_24_gambiae.csv"))
estimates_martin_24_funestus=read.csv(file.path(outputsDir, "estimates_martin_24_funestus.csv"))
...
all_estimates=rbind(estimates_kibondo_24 %>%mutate(EHT="Kibondo et al. 2022", EHT_short="Kibondo"),
                    ...
                    estimates_bit103_24 %>%mutate(EHT="Assenga et al. 2025", EHT_short="Assenga"),
                    estimates_bit103_24_CI %>%mutate(EHT="Assenga et al. 2025, CI", EHT_short="Assenga, CI"),
                    estimates_martin_24 %>%mutate(EHT="Martin et al. 2024", EHT_short="Martin"),
                    estimates_martin_24_funestus %>%mutate(EHT="Martin et al. 2024, f", EHT_short="Martin, f"),
                    impacts_24_summary 
)%>%
...
all_estimates$EHT=factor(all_estimates$EHT, levels=c("Assenga et al. 2025", "Kibondo et al. 2022", "BIT080", "BIT055", "Odufuwa et al. 2024", "Martin et al. 2024", "Martin et al. 2024, f", "Assenga et al. 2025, CI"))
...
ggsave(file.path(plotDir, "plot_EHTfit_pyr_all_24h.png"), width=12, height=10)
...
final_table %>%
  ...
  gt()%>%
  gt::gtsave(filename = file.path(plotDir, "outputs_ento_efficacy_all_24h.docx"))
