...
model{
  ...
  for (i in 1:N) {
    ...
    if (treat[i]==0) {
      ...
      theta[1,i] = (P_UA((1 -InitialRepellencyRate[treat[i]+1])*alpha_k0, mu_k0 + KillingDuringHostSeeking[treat[i]+1] * mu_k0));
      theta[2,i] = (P_UD((1 -InitialRepellencyRate[treat[i]+1])*alpha_k0,  mu_k0 + KillingDuringHostSeeking[treat[i]+1] * mu_k0));
      theta[3,i] = (P_FA((1 -InitialRepellencyRate[treat[i]+1])*alpha_k0,  mu_k0 + KillingDuringHostSeeking[treat[i]+1] * mu_k0, pB_k0*(1-InitialPostprandialkillingEfficacy[treat[i]+1])));
      theta[4,i] = (P_FD((1 -InitialRepellencyRate[treat[i]+1])*alpha_k0,  mu_k0 + KillingDuringHostSeeking[treat[i]+1] * mu_k0, pB_k0*(1-InitialPostprandialkillingEfficacy[treat[i]+1])));
    }
...
generated quantities {
  ...
  mu_i = mu_0 + KillingDuringHostSeeking * mu_0;
